...
\begin{proof}
	...
	\end{equation}
	Notably, this is exact because $\ZZ$ is a finitely generated, torsion-free $\ZZ$-module and hence flat as a $\ZZ$-module. Now, $\op{Hom}_\ZZ(X,\QQ)\otimes_\ZZ X$ is still a divisible abelian group, so again $\widehat H^i(G,\op{Hom}_\ZZ(X,\QQ))=0$ for all $i\in\ZZ$.
...
\begin{proof}
	...
	\[\begin{tikzcd}
		...
		{\widehat H^i(G,\ZZ)} & {\widehat H^2(G,X\otimes_\ZZ A)} & {\widehat H^{i+2}(G,A)}
		\arrow["{-\cup u}", from=1-1, to=1-2]
		\arrow["{-\cup u}", from=2-1, to=2-2]
	...
	\begin{enumerate}[label=(\arabic*)]
		...
		\item This square commutes by \autoref{lem:cupproductmorphism}.
		\item This square commutes by functoriality of $\widehat H^2(G,-)$ applied to \autoref{eq:innermorphismcoherence}.
	\end{enumerate}
...
\begin{cor}
	...
	\[\widehat H^2(G,A)\to\op{Hom}_\ZZ\left(\widehat H^{-2}(G,\op{Hom}_\ZZ(X,A)),\widehat H^0(G,\op{Hom}_\ZZ(X,A))\right).\]
\end{cor}
\begin{proof}
