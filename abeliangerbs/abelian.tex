...
\subsection{Tuple Relations}
The results from \autoref{sec:singlevar} are very focused on single elements $\sigma\in G$ and the cyclic groups they generate, but we will be more interested in dealing with more general abelian groups $G$. If we want to keep track of the fact our group is abelian, we should extract the elements of $A$ which can do so.
\begin{lemma}[{\cite[Lemma~1.2]{abelian-crossed}}] \label{lem:constructalphabeta}
	Let $G$ be an abelian group and $A$ be a $ G$-module, and let 
...
\begin{proof}
	...
	\begin{listalph}
		...
		so $\beta\in\ker\pi=A$. Note we have used the fact that $G$ is abelian.
		...
		\begin{align*}
			N_1(\beta) &= \sigma_1^{-1}(\beta)\cdot\sigma_1^{-2}(\beta)\cdot\sigma^{-3}(\beta)\cdot\ldots\cdot\sigma^{-n_1}(\beta) \\
...
\begin{lemma}[{\cite[Lemma~1.1(b)]{abelian-crossed}}] \label{lem:switchtwo}
	...
	\[F_1^xF_2^y=\prod_{k=0}^{x-1}\prod_{\ell=0}^{y-1}\left(\sigma_1^k\sigma_2^\ell(\beta)F_2^yF_1^x\right).\]
\end{lemma}
\begin{proof}
...
\end{remark}
Roughly speaking, \autoref{rem:alphabetarelation} tells us that coherence of the group law in $\mc E$ should give rise to relations between our elements of $A$. Namely, trying to expand out the group law as placed constraints on our $\alpha_1,\alpha_2,\beta$. Here is another example.
...
\end{proof}
\begin{remark}
	The relation from \autoref{lem:betarelations} may look asymmetric in the $\beta_{ij}$, but this is because the definitions of the $\beta_{ij}$s themselves are asymmetric in $F_i$. For example, swapping all $1$s and $2$s, will indeed recover the same relation.
\end{remark}
\begin{remark}
...
The preceding lemmas \autoref{lem:constructalphabeta} and \autoref{lem:betarelations} are intended to give intuition that the element $\beta$ is helping to specify the group law on $\mc E$.
...
\begin{cor} \label{cor:alltuplesfromextens}
	Fix everything as in the modified set-up, forgetting about the extension $\mc E$. Given any $\{\sigma_i\}_{i=1}^m$-tuple of $\{\alpha_i\}_{i=1}^m$ and $\{\beta_{ij}\}_{i>j}$, there exists an extension $\mc E$ and lifts $F_i$ of the $\sigma_i$ so that
